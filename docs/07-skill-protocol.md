...
插件沿用 ZIP 上传和审核流程。外层 ZIP 根目录只放 README.md 和一个非空的真实安装包；安装包文件按原字节保存，不递归解压。README 格式如下：
...
- 安装包支持 `.zip`、`.crx`、`.xpi`、`.vsix`、`.exe`、`.msi`、`.dmg`、`.pkg`、`.tgz`、`.tar.gz`。文件必须实际存在，文件名与 README 一致；版本和资源标识规则与网页资源一致。
- 总包大小沿用 100MB 上限；安装包可超过普通文件的 10MB 限制。README 和普通 Skill 文件仍受原有单文件限制。部署配置可进一步收紧限制。
- 发布版本的下载按钮返回原安装文件名和原始字节，不返回外层说明 ZIP。审核包下载仍返回完整外层 ZIP。
- 插件不进入 Skill CLI 搜索、列表或安装解析；详情页展示“下载安装包”，安装步骤由作者在 README 中说明。
- 扫描器尚未调整。原计划仅扫描 README，因自动审批要求进一步确认适用于所有插件的扫描范围，当前分支尚未完成插件扫描接入及真实上传验收。现有 Skill 和网页扫描保持原样。
