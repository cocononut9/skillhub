...
插件沿用 ZIP 上传和审核流程。外层 ZIP 根目录只放 README.md 和一个非空的真实安装包，安装包文件名与 README 声明一致。
...
- 支持 ZIP、CRX（2/3）、XPI、VSIX 安装包。版本和标识规则与网页一致；不检查 CRX 的签名有效性。
- 扫描时安全展开安装包及内部嵌套 ZIP 到临时副本，检查脚本、配置、文档和隐藏文件。原安装包不改变。
- 使用现有静态扫描引擎；JS、JSON、HTML 等通过同内容文本副本应用通用文本风险规则，Python/Shell 另保留原有语言规则。这不是专门的 JavaScript 漏洞分析或杀毒引擎。
- 图片（PNG/JPEG/GIF/WebP/ICO）只做格式检查；不检查图片内部隐藏载荷。含 EXE/DLL/WASM 等二进制程序、不可解开的归档、加密归档或未知二进制文件时扫描失败，不会作为安全通过。
- 整包最多 100MB；安装包可以超过普通文件的 10MB 限制。扫描展开及文本副本总量最多 100MB、归档项最多 500、嵌套层数最多 4，单个可读文本须小于 10MB。超限会明确失败。
- 只有当前扫描结果为 SAFE 才开放安装包下载和审核通过。管理员直接发布同样受下载限制；危险、失败及扫描中的版本都不能下载。旧扫描结果不会解锁新扫描轮次。
- 下载返回安装包原文件名、原始字节。审核人员下载完整审核包的已有权限保留，供人工检查。
- 插件不进入 Skill CLI 搜索、列表或安装解析。详情页展示“下载安装包”及静态扫描范围。
- 示例源码位于 `examples/plugin-resource/extension/`，可压缩为 `skillhub-demo.zip` 后与示例 README 一起打包上传。

## 提示词资源

上传 ZIP，根目录包含 `README.md` 和 `PROMPT.md`，也支持一层包裹目录。提示词正文不要求 Skill frontmatter，不需要 SKILL.md。
...
资源类型：提示词
资源标识：meeting-summary-prompt
版本：1.0.0

## 使用方法
复制提示词到 AI 工具，替换其中的输入占位符。
```

- 类型声明放在第一个二级标题之前；名称、简介、资源标识和版本沿用 README 规则。
- `PROMPT.md` 放完整原文，必须是非空 UTF-8 Markdown 文本，大小小于 10MB；不接受额外附件或二进制内容。
- README 和完整 PROMPT.md 均进入现有内容扫描。扫描副本临时添加 SKILL.md 引用正文，存储文件保持原样。
- 发布且当前扫描通过后，详情页展示提示词原文并提供“复制提示词”“下载 Markdown”；复制保留正文格式，下载返回原始 PROMPT.md 文件。未通过扫描不能获取正文或下载，归档后不提供复制和下载入口。
- 不显示 Skill 安装命令，不进入 Skill CLI 搜索、同步列表或安装解析。版本审核和权限沿用现有流程。
- 内容扫描采用现有规则，扫描通过不代表业务效果已经验证。示例位于 `examples/prompt-resource/`。
