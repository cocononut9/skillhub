        ...
        <>
          ...
          <div className="max-h-64 space-y-3 overflow-y-auto p-1">
            {(['WORKFLOW', 'ROLE', 'GENERAL'] as const).map((category) => {
              const options = filtered.filter((label) => (label.category ?? 'GENERAL') === category)
              if (options.length === 0) return null
              return (
                <div key={category} role="group" aria-label={t('labelCategories.' + category)} className="space-y-2">
                  <div className="text-sm font-medium">{t('labelCategories.' + category)}</div>
