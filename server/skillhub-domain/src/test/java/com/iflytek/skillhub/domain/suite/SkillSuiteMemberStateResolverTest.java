...
class SkillSuiteMemberStateResolverTest {
    ...
    void appliesCanonicalSkillVisibilityRulesToMemberBrowsing() {
        ...
        assertThat(skillOwner.downloadReady()).isTrue();
        for (ResourceType type : List.of(ResourceType.WEB, ResourceType.PLUGIN, ResourceType.PROMPT)) {
            skill.setResourceType(type);
            var state = resolver.resolveForViewer(
                    List.of(member), "skill-owner", Map.of(1L, NamespaceRole.MEMBER), Set.of()).getFirst();
            assertThat(state.downloadReady()).as("%s must not be installed as a Suite member", type).isFalse();
        }
    }
