...
@Import({PostgresResourceDiscoveryQueryService.class, ResourceDiscoveryAppService.class,
        MySkillSuiteQueryRepository.class, SkillSuiteReferenceQueryRepository.class, SkillSuiteCandidateQueryRepository.class})
@Testcontainers
...
class SuiteDiscoveryIntegrationTest {
    ...
    @Autowired
    private SkillSuiteCandidateQueryRepository candidateRepository;

    @Test
    void suiteCandidatesExcludeNonInstallableResourceTypes() {
        Namespace namespace = entityManager.persistFlushFind(new Namespace("candidates", "Candidates", "owner"));
        ...
        assertThat(candidateRepository.search(namespace.getId(), SkillVisibility.PUBLIC, "resource-", "owner",
                List.of(), List.of(), true, 20)).extracting(candidate -> candidate.slug())
                .containsExactly("resource-skill");
    }
